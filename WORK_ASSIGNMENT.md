...
## Thành viên 2 — Task 3 (Security/Quality): SonarCloud + evidence (screenshot)

### Mục tiêu
- Bật SonarCloud scan trên PR và có Quality Gate (gating) theo yêu cầu đề nâng cao.
...
- Tạo workflow scan SonarCloud (hoặc tích hợp vào workflow hiện có), chạy trên `pull_request` vào `develop`/`main`.
- Bật Quality Gate và chứng minh nó hoạt động (PR fail nếu gate fail).

### Evidence cần chụp (để nộp)
...
- Mở PR vào `develop` và chờ CI pass (`CI Gate`).
...
- Chạy local (Maven Wrapper):
  - `./mvnw -B -ntp -f ../pom.xml -pl <module> -am test`
  - `./mvnw -B -ntp -f ../pom.xml -pl <module> jacoco:report`
- PR vào `develop` (đúng repo `PeriodicallyZoneOut/yas`).
